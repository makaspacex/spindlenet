...
class RenderEngineForMTH(object):
    def __init__(this,cs=320,os=128,fos=32):
        this.CS=cs;
        this.os=os;
        this.fos=fos;
    ...
    def center_draw(this, what,font: ImageFont):
        img = np.zeros([this.CS,this.CS,3],dtype=np.uint8);
        img=Image.fromarray(img);
        draw = ImageDraw.Draw(img)
    ...
    def render_core(this,charset,sp_tokens,font_paths,save_clip=False):
        magic = {}
        ...
        ttfons = []
        for _f_path in font_paths:
            ttfons.append(TTFont(_f_path))
        
        for i, char in enumerate(charset):
            
            f_path = font_paths[0]
            for ii, ttfont in enumerate(ttfons):
                rr = find_char_in_fonts(char=char, ttfonts=ttfont)
                if rr:
                    f_path = font_paths[ii]
                    break
            imfont = ImageFont.truetype(f_path, 64)
            protol = this.center_draw( char, imfont)
            
            if (save_clip):
                cv2.imwrite("im" + str(ord(char[0])) + ".jpg", protol);
            chars.append(char);
            if (i % 500 == 0):
                print(i, "of", len(charset));
            protos.append(torch.tensor([protol[:, :, 0:1]]).float().permute(0, 3, 1, 2).contiguous());
...
if __name__ == '__main__':
    ...
    # sp_tokens=["[GO]","[s]"];
    rlt=RenderLite();
    im=rlt.center_draw(64,"嗢","/home/izhangxm/Desktop/work/main/VSDF/ddata/fonts/NotoSansBengali-Regular.ttf");
    cv2.imshow("meow",im);
    cv2.waitKey(0);
