...
class NekoCAMStop(nn.Module):
    def __init__(self, scales, maxT, depth, num_channels):
        ...
        for i in range(1, len(scales)):
            assert (
                not (scales[i - 1][1] / scales[i][1]) % 1
            ), "layers scale error, from {} to {}".format(i - 1, i)
            assert (
                not (scales[i - 1][2] / scales[i][2]) % 1
            ), "layers scale error, from {} to {}".format(i - 1, i)
            ksize = [
                3,
                3,
                5,
            ]  # if downsampling ratio >= 3, the kernel size is 5, else 3
            r_h, r_w = int(scales[i - 1][1] / scales[i][1]), int(
                scales[i - 1][2] / scales[i][2]
            )
        ...
        for i in range(1, int(depth / 2)):
            deconvs.append(
                nn.Sequential(
                    nn.ConvTranspose2d(
                        ...
                        (
                            int(deconv_ksizes[int(depth / 2) - i][0] / 4.0),
                            int(deconv_ksizes[int(depth / 2) - i][1] / 4.0),
                        ),
                    ),
                    nn.BatchNorm2d(num_channels),
                    nn.ReLU(True),
            ...
            ich_stop += num_channels
        self.lenpred = nn.Linear(ich_stop, maxT, False)
        deconvs.append(
            nn.Sequential(
                nn.ConvTranspose2d(
                    num_channels,
                    ...
                    tuple(strides[0]),
                    (int(deconv_ksizes[0][0] / 4.0), int(deconv_ksizes[0][1] / 4.0)),
                ),
                nn.Sigmoid(),
            )
        )
    ...
    def forward(self, input):
        ...
        leng = self.lenpred(Ts)
        x = self.deconvs[-1](x)
        return x, leng


neko_CAM_stop = NekoCAMStop
