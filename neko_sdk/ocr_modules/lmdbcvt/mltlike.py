...
class ImgtLmdbexport:
    ...
    def __init__(this,dst):
        ...
        os.makedirs(dst);
        this.db=ImLmdbWrapper(dst);
        # this.spc=torch.load("spcmlt.pt");
        pass;

class MltLmdbexport(ImgtLmdbexport):
...
class MltSynthExport(ImgtLmdbexport):
    def setup(this,**kwargs):
...
class LsvtLmdbexport(ImgtLmdbexport):
    def handle_line(this,im,line):
...
class RctwLmdbexport(ImgtLmdbexport):
    def handle_line(this,im,line):
        cord,content=line.split(",\"",1);
...
def make_mlt_valjp(src,dst):
    expo = MltLmdbexport(dst);
