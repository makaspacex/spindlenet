...
class Loss_counter():
    ...
    def get_loss_and_terms(self):
        loss = self.loss_sum / self.total_iters if self.total_iters > 0 else 0
        retterms = {}
        for k in self.termsum:
            term = self.termsum[k] / self.total_iters if self.total_iters > 0 else 0
            retterms[k] = term
        self.clear()
...
class neko_oswr_Attention_AR_counter():
    ...
    def add_iter(self, prdt_texts, label_length, labels, debug=False):
        ...
        for i in range(0, len(prdt_texts)):
            ...
            if (labels[i].find("⑨") != -1):
                self.total_U += 1
                self.Ucorr += (prdt_texts[i].find("⑨") != -1)
            else:
                self.total_K += 1
                self.Kcorr += cflag
                self.KtU += (prdt_texts[i].find("⑨") != -1)
