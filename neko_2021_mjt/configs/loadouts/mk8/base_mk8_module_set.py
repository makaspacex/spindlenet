...
def arm_trinorm_mk8_common_nota(srcdst, prefix, capacity, feat_ch, tr_meta_path, expf=1, views=["synthw", "glyph"],
                                ccnt=3900, camch=64):
...
def arm_trinorm_mk8h_common_nota(srcdst, prefix, capacity, feat_ch, tr_meta_path, expf=1, views=["synthw", "glyph"],
                                 ccnt=3900, camch=64):
    srcdst = arm_trinorm_mk8_common_noa(srcdst, prefix, capacity, feat_ch, tr_meta_path, expf=expf, views=views,
                                        ccnt=ccnt, camch=camch)
...
def arm_trinorm_mk8h_common_ptpt_nota(srcdst, prefix, capacity, feat_ch, tr_meta_path, expf=1,
...
def arm_trinorm_mk8_common(srcdst, prefix, maxT, capacity, feat_ch, tr_meta_path, expf=1, views=["synthw", "glyph"],
                           ccnt=3900, camch=64):
...
def arm_trinorm_mk8h_common(srcdst, prefix, maxT, capacity, feat_ch, tr_meta_path, expf=1, views=["synthw", "glyph"],
                            ccnt=3900, camch=64):
    srcdst = arm_trinorm_mk8h_common_nota(srcdst, prefix, capacity, feat_ch, tr_meta_path, expf=expf, views=views,
                                          ccnt=ccnt, camch=camch)
    if (maxT == 1):
        maxT = 2
    srcdst[prefix + "TA"] = config_cam_stop(maxT, feat_ch=feat_ch, scales=[
...
def arm_trinorm_mk8h_ptpt_common(srcdst, prefix, maxT, capacity, feat_ch, tr_meta_path, expf=1,
                                 views=["synthw", "glyph"], ccnt=3900, camch=64):
    srcdst = arm_trinorm_mk8h_common_ptpt_nota(srcdst, prefix, capacity, feat_ch, tr_meta_path, expf=expf, views=views,
                                               ccnt=ccnt, camch=camch)
    if (maxT == 1):
        maxT = 2
    srcdst[prefix + "TA"] = config_cam_stop(maxT, feat_ch=feat_ch, scales=[
...
def arm_trinorm_mk8hnp_module_set_dan_r45ptpt(srcdst, prefix, maxT, capacity, feat_ch, tr_meta_path, expf=1,
                                              views=["synthw", "glyph"], ccnt=3900, camch=64, sheads=8, slays=4,
                                              wemb=0.3):
    srcdst = arm_trinorm_mk8h_ptpt_common(srcdst, prefix, maxT, capacity, feat_ch, tr_meta_path, expf=expf, views=views,
                                          ccnt=ccnt, camch=camch)
...
def arm_trinorm_mk8hnp_module_set_dan_r45(srcdst, prefix, maxT, capacity, feat_ch, tr_meta_path, expf=1,
                                          views=["synthw", "glyph"], ccnt=3900, camch=64, sheads=8, slays=4, wemb=0.3):
    srcdst = arm_trinorm_mk8h_common(srcdst, prefix, maxT, capacity, feat_ch, tr_meta_path, expf=expf, views=views,
                                     ccnt=ccnt, camch=camch)
    srcdst = arm_mk8_proto_sganp(srcdst, prefix, capacity, feat_ch)
    srcdst = arm_rest_commonwops(srcdst, prefix, wemb=wemb)
    srcdst = arm_mk8_ctx(srcdst, prefix, feat_ch, sheads, slays)
...
def arm_base_mk8_eval_routine(srcdst, tname, prefix, routine_type, log_path, maxT, view_name, proto_viewname,
                              force_skip_ctx, measure_rej):
    if (force_skip_ctx):
...
def arm_base_mk8_task_default(srcdst, prefix, routine_type, maxT, te_meta_path, datasets, log_path,
                              name="close_set_benchmarks", view_name="synthw", proto_viewname="glyph",
                              force_skip_ctx=False, measure_rej=False):
    te_routine = {}
