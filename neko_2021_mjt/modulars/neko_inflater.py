...
class NekoInflater(object):
    ...
    def prob_length(self, out_res, nsteps, nB):
        out_length = torch.zeros(nB)
        for i in range(0, nsteps):
            tens = out_res[i, :, :]
            tmp_result = tens.topk(1)[1].NekoSqueeze(-1)
            for j in range(nB):
    ...
    def inflate(self, out_emb, out_length):
        nT, nB = out_emb.shape[0], out_emb.shape[1]
        start = 0
        if (out_length is None):
            out_length = self.prob_length(out_emb, nT, nB)
    ...
    # def inflate(self,out_emb,out_length):
    #     output1, out_length1=self.inflate1(out_emb,out_length)
    #     output2, out_length2 = self.inflate2(out_emb, out_length)
