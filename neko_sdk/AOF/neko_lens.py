...
class neko_dense_calc(nn.Module):
    ...
    def forward(self, feat, lw, lh):
        if (self.training):
            dmap = self.dkern(feat / (feat.norm(dim=1, keepdim=True) + 0.00000009))
        else:
            norm = feat.norm(dim=1, keepdim=True)
            dmap = self.dkern(feat / (norm + 0.00000009) * (norm > 0.09))
...
class neko_lens(nn.Module):
    ...
    def __init__(self, channel, pw, ph, hardness=2, dbg=False):
        ...
        self.dkern = self.DENSE(channel)

    def forward(self, feat):
        dw = feat.shape[3] // self.pw
        dh = feat.shape[2] // self.ph
        dmap = self.dkern(feat, dw, dh)
        dmap += 0.0009;  # sigmoid can be clipped to 0!
        dmap[:, :, 1:-1, 1:-1] += self.hardness
...
class neko_lens_self(nn.Module):
    def __init__(self, ich, channel, pw, ph, hardness=2, dbg=False, scale=None):
        ...
        if (type(hardness) is not tuple):
            ...
            self.hardness = nn.Parameter(torch.tensor(hardness).reshape(1, -1, 1, 1), requires_grad=False)
        self.ekern = torch.nn.Sequential(
    ...
    def forward_dsz(self, feat, dsz):
        dw = feat.shape[3] // self.pw
        dh = feat.shape[2] // self.ph
        lfeat = self.ekern(feat)
        dmap = self.dkern(lfeat, dw, dh)
        dmap += 0.0009;  # sigmoid can be clipped to 0!
        dmap[:, :, 1:-1, 1:-1] += self.hardness
        # self.scale=0.25
        if (dsz is not None):
    ...
    def forward(self, feat):
        dsz = [int(feat.shape[2] * self.scale[0]), int(feat.shape[3] * self.scale[1])]
...
class neko_lens_fuse(nn.Module):
    ...
    def forward(self, hifeat, lofeat):
        dw = lofeat.shape[3]
        dh = lofeat.shape[2]
        lw = hifeat.shape[3]
        lh = hifeat.shape[2]
        hidmap = self.hidense(hifeat, lw, lh)
        lodmap = self.lodense(lofeat, lw, lh)
