    ...
    def evaluate_image(self, gt, pred):
        ...
        def one_to_one_match(row, col):
            ...
            for j in range(len(recallMat[0])):
                if recallMat[row, j] >= self.area_recall_constraint and precisionMat[
            ...
            for i in range(len(recallMat)):
                if recallMat[i, col] >= self.area_recall_constraint and precisionMat[
                    i, col] >= self.area_precision_constraint:
                    cont = cont + 1
            if (cont != 1):
                return False

            if recallMat[row, col] >= self.area_recall_constraint and precisionMat[
        ...
        if len(detRects) > 0:
                ...
                if gtNum not in gtDontCareRectsNum:
                    ...
                    if match is True:
                        ...
                        if num_overlaps_gt(gtNum) >= 2:
                            ...
                            precisionAccum += (
                                self.mtype_oo_o if len(matchesDet) == 1 else self.mtype_om_o * len(matchesDet))
                            pairs.append(
                                {'gt': gtNum, 'det': matchesDet, 'type': 'OO' if len(matchesDet) == 1 else 'OM'})
                ...
                if detNum not in detDontCareRectsNum:
                    ...
                    if match is True:
                        ...
                        if num_overlaps_det(detNum) >= 2:
                            detRectMat[detNum] = 1
                            recallAccum += (
                                self.mtype_oo_o if len(matchesGt) == 1 else self.mtype_om_m * len(matchesGt))
                            ...
                            evaluationLog += "Match Discarded GT #" + str(matchesGt) + " with Det #" + str(
                                detNum) + " not single overlap\n"
            ...
            if numGtCare == 0:
                ...
                precision = float(0) if (len(detRects) - len(detDontCareRectsNum)) == 0 else float(precisionAccum) / (
