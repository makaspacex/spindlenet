...
class NekoAbstractRoutine(object):
    ...
    def fp_impl(self, input_dict, modular_dict, logger_dict, nEpoch, batch_idx):
        return torch.tensor(0)

    def bp_impl(self, loss):
        loss.backward()

    def fpbp_impl(self, input_dict, modular_dict, logger_dict, nEpoch, batch_idx):
        loss = self.fp_impl(input_dict, modular_dict, logger_dict, nEpoch, batch_idx)
        self.bp_impl(loss)
        pass

    def fpbp_amp_impl(self, input_dict, modular_dict, logger_dict, nEpoch, batch_idx):
        with torch.cuda.amp.autocast():
            loss = self.fp_impl(input_dict, modular_dict, logger_dict, nEpoch, batch_idx)
        self.bp_impl(loss)
        pass

    def fp(self, input_dict, modular_dict, nEpoch, batch_idx):
        idict, mdict = self.grab_modules(input_dict, modular_dict)
        loss = self.fp_impl(idict, mdict, self.logger_dict, nEpoch, batch_idx)
        if (batch_idx % self.log_each == 0):
            self.log_show()
        return loss

    def fpbp(self, input_dict, modular_dict, nEpoch, batch_idx):
        idict, mdict = self.grab_modules(input_dict, modular_dict)
        ...
        ret = self.fpbp_impl(idict, mdict, self.logger_dict, nEpoch, batch_idx)
        if (batch_idx % self.log_each == 0):
            self.log_show()
        return ret

    def fpbp_amp(self, input_dict, modular_dict, nEpoch, batch_idx):
        idict, mdict = self.grab_modules(input_dict, modular_dict)
        ret = self.fpbp_amp_impl(idict, mdict, self.logger_dict, nEpoch, batch_idx)
        if (batch_idx % self.log_each == 0):
            self.log_show()
        return ret
...
class NekoAbstractEvalRoutine(object):
